---
title: The Ultimate Guide to Getting Started with the Rollup.js JavaScript Bundler
categories:
  - "[[Resources]]"
created: 2026-02-16
published: 2021-10-07
source: https://medium.com/stackanatomy/the-ultimate-guide-to-getting-started-with-the-rollup-js-javascript-bundler-2ebec9398656
author:
  - "[[Craig Buckler]]"
description: "Ultimate Guide to Rollup.js "
tags:
  - tech/rollup
---

![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*Xq8dmOc6Q8pE27FsRROirg.jpeg)
...
4. Unused functions are automatically removed using *tree-shaking* methods which reduce file sizes and improve performance.
...
- `--format <type>` (or `-f`): the JavaScript bundle format:  
	`iife` output an Immediately Invoked Function Expression `(function () { ... }());   es` standard ES6  
...
```c
// rollup.config.js
import replace from '@rollup/plugin-replace';const devMode = (process.env.NODE_ENV === 'development');
console.log(\`${ devMode ? 'development' : 'production' } mode bundle\`);export default [
...
import commonjs from '@rollup/plugin-commonjs';const devMode = (process.env.NODE_ENV === 'development');
console.log(\`${ devMode ? 'development' : 'production' } mode bundle\`);export default [
  {    input: './src/main.js',    plugins: [
      nodeResolve(),
      commonjs()
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/0*QDYEeQXbTjQarz4R.png)
...
You can transpile your code to ES5 using [Babel](https://babeljs.io/) if you’re unfortunate enough to have a reasonably high number of IE11 users. The resulting code *should* work in any browser (although you may still require Polyfills to add missing functionality in IE11).
...
```c
// rollup.config.js
import { getBabelOutputPlugin } from '@rollup/plugin-babel';const devMode = (process.env.NODE_ENV === 'development');
console.log(\`${ devMode ? 'development' : 'production' } mode bundle\`);export default [
  {
    // ES6
    input: './src/main.js',    output: {
...
*Originally published at* [*https://blog.openreplay.com*](https://blog.openreplay.com/the-ultimate-guide-to-getting-started-with-the-rollup-js-javascript-bundler) *on October 7, 2021.*

[![StackAnatomy](https://miro.medium.com/v2/resize:fill:48:48/1*rMfOrzM5XI3hEGXX_4nxaQ.png)](https://medium.com/stackanatomy?source=post_page---post_publication_info--2ebec9398656---------------------------------------)

[![StackAnatomy](https://miro.medium.com/v2/resize:fill:64:64/1*rMfOrzM5XI3hEGXX_4nxaQ.png)](https://medium.com/stackanatomy?source=post_page---post_publication_info--2ebec9398656---------------------------------------)

[Last published Sep 1, 2024](https://medium.com/stackanatomy/better-error-handling-with-monads-part-2-ca05e20e3a14?source=post_page---post_publication_info--2ebec9398656---------------------------------------)

Quality content for front-end developers

Does web tech dev stuff and writes about it now and again.

## More from Craig Buckler and StackAnatomy
...
](https://medium.com/?source=post_page---read_next_recirc--2ebec9398656---------------------------------------)