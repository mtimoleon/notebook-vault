...
## Ledgers, Facts, and States[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#ledgers-facts-and-states)
...
## The Ledger[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#the-ledger)
...
![ledger.ledger](Ledger%20—%20Hyperledger%20Fabric%20Docs%20main%20documentation-20260217%204.png)

_A Ledger L comprises blockchain B and world state W, where blockchain B determines world state W. We can also say that world state W is derived from blockchain B._  
...
## World State[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#world-state)
...
![ledger.worldstate](Ledger%20—%20Hyperledger%20Fabric%20Docs%20main%20documentation-20260217%201.png)

_A ledger world state containing two states. The first state is: key=CAR1 and value=Audi. The second state has a more complex value: key=CAR2 and value={model:BMW, color=red, owner=Jane}. Both states are at version 0._  
...
Applications submit transactions which capture changes to the world state, and these transactions end up being committed to the ledger blockchain. Applications are insulated from the details of this [consensus](https://hyperledger-fabric.readthedocs.io/en/release-2.2/txflow.html) mechanism by the Hyperledger Fabric SDK; they merely invoke a smart contract, and are notified when the transaction has been included in the blockchain (whether valid or invalid). The key design point is that only transactions that are **signed** by the required set of **endorsing organizations** will result in an update to the world state. If a transaction is not signed by sufficient endorsers, it will not result in a change of world state. You can read more about how applications use [smart contracts](https://hyperledger-fabric.readthedocs.io/en/release-2.2/smartcontract/smartcontract.html), and how to [develop applications](https://hyperledger-fabric.readthedocs.io/en/release-2.2/developapps/developing_applications.html).  
...
## Blockchain[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#blockchain)
...
![ledger.blockchain](Ledger%20—%20Hyperledger%20Fabric%20Docs%20main%20documentation-20260217%203.png)

_A blockchain B containing blocks B0, B1, B2, B3. B0 is the first block in the blockchain, the genesis block._  
...
Finally, as you can see in the diagram, the first block in the blockchain is called the **genesis block**. It’s the starting point for the ledger, though it does not contain any user transactions. Instead, it contains a configuration transaction containing the initial state of the network channel (not shown). We discuss the genesis block in more detail when we discuss the blockchain network and [channels](https://hyperledger-fabric.readthedocs.io/en/release-2.2/channels.html) in the documentation.

## Blocks[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#blocks)
...
![ledger.blocks](Ledger%20—%20Hyperledger%20Fabric%20Docs%20main%20documentation-20260217%202.png)

_Block header details. The header H2 of block B2 consists of block number 2, the hash CH2 of the current block data D2, and the hash of the prior block header H1._

- **Block Data**

This section contains a list of transactions arranged in order. It is written when the block is created by the ordering service. These transactions have a rich but straightforward structure, which we describe [later](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#transactions) in this topic.
...
## Transactions[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#transactions)
...
![ledger.transaction](Ledger%20—%20Hyperledger%20Fabric%20Docs%20main%20documentation-20260217%205.png)

_Transaction details. Transaction T4 in blockdata D1 of block B1 consists of transaction header, H4, a transaction signature, S4, a transaction proposal P4, a transaction response, R4, and a list of endorsements, E4._  
...
## World State database options[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#world-state-database-options)
...
CouchDB is a particularly appropriate choice when ledger states are structured as JSON documents because CouchDB supports the rich queries and update of richer data types often found in business transactions. Implementation-wise, CouchDB runs in a separate operating system process, but there is still a 1:1 relation between a peer node and a CouchDB instance. All of this is invisible to a smart contract. See [CouchDB as the StateDatabase](https://hyperledger-fabric.readthedocs.io/en/release-2.2/couchdb_as_state_database.html) for more information on CouchDB.  
In LevelDB and CouchDB, we see an important aspect of Hyperledger Fabric – it is _pluggable_. The world state database could be a relational data store, or a graph store, or a temporal database. This provides great flexibility in the types of ledger states that can be efficiently accessed, allowing Hyperledger Fabric to address many different types of problems.

## Example Ledger: Basic Asset Transfer[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#example-ledger-basic-asset-transfer)

As we end this topic on the ledger, let’s have a look at a sample ledger. If you’ve run the [basic asset transfer sample application](https://hyperledger-fabric.readthedocs.io/en/release-2.2/write_first_app.html), then you’ve created this ledger.  
...
![ledger.transaction](Ledger%20—%20Hyperledger%20Fabric%20Docs%20main%20documentation-20260217.png)

_The ledger, L, comprises a world state, W and a blockchain, B. W contains four states with keys: ASSET1, ASSET2, ASSET3, and ASSET4. B contains two blocks, 0 and 1. Block 1 contains four transactions: T1, T2, T3, T4._  
...
## Namespaces[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#namespaces)
...
A blockchain is not namespaced. It contains transactions from many different smart contract namespaces. You can read more about chaincode namespaces in this [topic](https://hyperledger-fabric.readthedocs.io/en/release-2.2/developapps/chaincodenamespace.html).  
Let’s now look at how the concept of a namespace is applied within a Hyperledger Fabric channel.

## Channels[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#channels)

In Hyperledger Fabric, each [channel](https://hyperledger-fabric.readthedocs.io/en/release-2.2/channels.html) has a completely separate ledger. This means a completely separate blockchain, and completely separate world states, including namespaces. It is possible for applications and smart contracts to communicate between channels so that ledger information can be accessed between them.  
You can read more about how ledgers work with channels in this [topic](https://hyperledger-fabric.readthedocs.io/en/release-2.2/developapps/chaincodenamespace.html#channels).

## More information[¶](https://hyperledger-fabric.readthedocs.io/en/release-2.2/ledger/ledger.html#more-information)

See the [Transaction Flow](https://hyperledger-fabric.readthedocs.io/en/release-2.2/txflow.html), [Read-Write set semantics](https://hyperledger-fabric.readthedocs.io/en/release-2.2/readwrite.html) and [CouchDB as the StateDatabase](https://hyperledger-fabric.readthedocs.io/en/release-2.2/couchdb_as_state_database.html) topics for a deeper dive on transaction flow, concurrency control, and the world state database.