...
- The duration-related edges of the dependency graph are added as follows. For each operation A:
	...
	o If A’s duration matches that of another operation B, an edge is created Bd -> Ad.
	o If A’s duration is set to the difference between the end of operation C and the start of operation B, then three edges as created as follows: Bs -> Ad, Cs -> Ad, Cd -> Ad.
...
- For each node of the list we do the following:
	o If it is a duration node (e.g. Ad):
		...
		Β§ If A’s duration matches that of another operation B, it is set to Bd.
		Β§ If A’s duration is set to the difference between the end of operation C and the start of operation B, then it is set to (Cs + Cd) – Bs.
