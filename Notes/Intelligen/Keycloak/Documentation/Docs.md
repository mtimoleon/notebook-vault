...
A good practice is to include the JavaScript adapter in your application using a package manager like NPM or Yarn. TheΒ keycloak-jsΒ package is available on the following locations:

- NPM:Β  [https://www.npmjs.com/package/keycloak-js](https://www.npmjs.com/package/keycloak-js)
- Yarn:Β  [https://yarnpkg.com/package/keycloak-js](https://yarnpkg.com/package/keycloak-js)

Alternatively, the library can be retrieved directly from the Keycloak server atΒ /js/keycloak.jsΒ and is also distributed as a ZIP archive.  
...
To use the JavaScript adapter you must first create a client for your application in the Keycloak Admin Console. Make sureΒ publicΒ is selected forΒ Access Type. You achieve this in Capability config by turning OFF client authentication toggle.  
==You also need to configure==Β ==Valid Redirect URIs==Β ==and==Β ==Web Origins. Be as specific as possible as failing to do so may result in a security vulnerability.==  
Once the client is created click on theΒ ActionΒ tab in the upper right corner and selectΒ Download adapter config. SelectΒ Keycloak OIDC JSONΒ forΒ Format OptionΒ then clickΒ Download. The downloadedΒ keycloak.jsonΒ file should be hosted on your web server at the same location as your HTML pages.  
...
[OpenID Connect](https://openid.net/connect/)Β (OIDC) is an authentication protocol that is an extension ofΒ [OAuth 2.0](https://datatracker.ietf.org/doc/html/rfc6749). While OAuth 2.0 is only a framework for building authorization protocols and is mainly incomplete, OIDC is a full-fledged authentication and authorization protocol. OIDC also makes heavy use of theΒ [Json Web Token](https://jwt.io/)Β (JWT) set of standards. These standards define an identity token JSON format and ways to digitally sign and encrypt that data in a compact and web-friendly way.  
There are really two types of use cases when using OIDC. The first is an application that asks the Keycloak server to authenticate a user for them. After a successful login, the application will receive anΒ _identity token_Β and anΒ _access token_. TheΒ _identity token_Β contains information about the user such as username, email, and other profile information. TheΒ _access token_Β is digitally signed by the realm and contains access information (like user role mappings) that the application can use to determine what resources the user is allowed to access on the application.  
==The second type of use cases is that of a client that wants to gain access to remote services. In this case, the client asks Keycloak to obtain an==Β _access token_Β ==it can use to invoke on other remote services on behalf of the user. Keycloak authenticates the user then asks the user for consent to grant access to the client requesting it. The client then receives the==Β _access token_==. This==Β _access token_Β ==is digitally signed by the realm. The client can make REST invocations on remote services using this==Β _access token_==. The REST service extracts the==Β _access token_==, verifies the signature of the token, then decides based on access information within the token whether or not to process the request.==
...
**Logging in**  
The end user interacts with a relying party (such as a website) that provides an option to specify an OpenID for the purposes of authentication; an end user typically has previously registered an OpenID (e.g.Β alice.openid.example.org) with an OpenID provider (e.g.==Β openid.example.org).1  
The relying party typically transforms the OpenID into a canonical URL form (e.g.Β http://alice.openid.example.org/).

- With OpenID 1.0, the relying party then requests the HTML resource identified by the URL and reads an HTML link tag to discover the OpenID provider's URL (e.g.Β http://openid.example.org/openid-auth.php). The relying party also discovers whether to use aΒ delegated identityΒ (see below).
- With OpenID 2.0, the relying party discovers the OpenID provider URL by requesting theΒ XRDSΒ documentΒ (also called theΒ YadisΒ document) with the content typeΒ application/xrds+xml; this document may be available at the target URL and is always available for a target XRI.==
...
TheΒ checkid_immediateΒ mode can fall back to theΒ checkid_setupΒ mode if the operation cannot be automated.  
First, the relying party and the OpenID provider (optionally) establish aΒ shared secret, referenced by an==Β associate handle, which the relying party then stores. If using theΒ checkid_setupΒ mode, the relying party redirects the end user's user-agent to the OpenID provider so the end user can authenticate directly with the OpenID provider.  
...
If the end user accepts the OpenID provider's request to trust the relying party, then the user-agent is redirected back to the relying party along with the end user's credentials. That relying party must then confirm that the credentials really came from the OpenID provider. If the relying party and OpenID provider had previously established a shared secret, then the relying party can validate the identity of the OpenID provider by comparing its copy of the shared secret against the one received along with the end user's credentials; such a relying party is calledΒ statefulΒ because it stores the shared secret between sessions. In contrast, aΒ statelessΒ orΒ dumbΒ relying party must make one more background request (check_authentication) to ensure that the data indeed came from the OpenID provider.  
After the OpenID has been verified, authentication is considered successful and the end user is considered logged into the relying party under the identity specified by the given OpenID (e.g.Β alice.openid.example.org). The relying party typically then stores the end user's OpenID along with the end user's other session information.==  
**Identifiers**  
To obtain an OpenID-enabledΒ URLΒ that can be used to log into OpenID-enabled websites, a user registers an OpenID identifier with an identity provider. Identity providers offer the ability to register a URL (typically a third-level domain, e.g. username.example.com) that will automatically be configured with OpenID authentication service.  
Once they have registered an OpenID, a user can also use an existing URL under their own control (such as a blog or home page) as an alias or "delegated identity". They simply insert the appropriate OpenID tags in theΒ HTML13==Β or serve aΒ YadisΒ document.14==  
...
XRIsΒ are a new form ofΒ InternetΒ identifierΒ designed specifically for cross-domain digital identity. For example, XRIs come in two formsβ€”i-namesΒ andΒ i-numbersβ€”that are usually registered simultaneously asΒ synonyms. I-names are reassignable (like domain names), while i-numbers are never reassigned. When an XRI i-name is used as an OpenID identifier, it is immediately resolved to the synonymous i-number (the CanonicalID element of the XRDS document). This i-number is the OpenID identifier stored by the relying party. In this way, both the user and the relying party are protected from the end user's OpenID identity ever being taken over by another party as can happen with a URL based on a reassignable DNS name.
