...
3. Resource reallocation: αλλαγή main equipment σε compatible (αν χωράει καθαρά). TryResourceReallocationForConflict
4. Breaks σε operations αν το επιτρέπουν τα flags (ShiftOrBreakForEquipment) και το conflict δεν ξεκινά/τελειώνει μέσα σε operation. TryAddingBreaksForConflict
5. Flex shifting: μετακίνηση procedure/operation σε διαθέσιμο slot forward/backward. TryFlexShiftingForConflict
6. Batch shifting: αναζήτηση equipment που απαιτεί ελάχιστη μετατόπιση batch. TryBatchShiftingForConflict
...
• Batch shifting: υπολογισμός batch shift όπως στα main equipment.
...
• Batch shifting: αναζήτηση slot (ή σε pool όταν χρειάζεται).
SchedulingService.cs
9) Labor (consumable)
• Overuse / Outage
• Breaks: αν ShiftOrBreakForLabor.
• Flex shifting: slot forward/backward με rate.
• Batch shifting: αναλογικά με staff/equipment, αλλά με rate.
SchedulingService.cs
...
• Batch shifting: παρόμοια λογική, με σεβασμό MaintainBatchIntegrity.
SchedulingService.cs
...
• Αποφεύγει λύσεις που μετακινούν precedent task όταν δεν πρέπει.
SchedulingService.cs
...
• Μετά από επιτυχή επίλυση, γίνεται UpdatePostProcessingTasks + UpdateConditionalTasks.
SchedulingService.cs
